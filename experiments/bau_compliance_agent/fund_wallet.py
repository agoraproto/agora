"""Sprint 21 — fund the Bau-Compliance Agent wallet + refresh listing.json.

Same fix-up logic as audit_agent/fund_wallet.py:
  1. Send 0.001 ETH from deployer to the agent's wallet for gas
  2. Refresh data/listing.json against the live API

Run once on the server (or re-run any time the wallet is low):
...
MIN_BALANCE_ETH = Decimal("0.0005")
...
def refresh_listing(did: str) -> None:
    ...
    r = httpx.get(
        f"{API}/v1/listings",
        params={"seller_did": did, "service_capability": CAPABILITY},
        timeout=15,
    )
...
def main() -> None:
    ...
    did = creds["did"]
    print(f"=== Bau-Compliance Agent: {did} ===")
    print(f"=== Wallet: {addr} ===")
    ...
    print("\nDone. The agent will pick up new jobs on its next poll cycle.")
