"""Sprint 31b — Provider-side RFQ listener for the German Building Compliance Check.
...
log = logging.getLogger("bau_compliance_agent.rfq")
...
class RfqListener:
    ...
    async def _submit_bid(self, http: httpx.AsyncClient, req: dict[str, Any]) -> None:
        ...
        body = {
            ...
            "currency": "USDC",
            "message": (
                f"Bau-Compliance check for {req.get('capability', 'AuditDocumentGapCheck')}. "
...
async def main() -> None:
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [bau-rfq] %(message)s",
        datefmt="%H:%M:%S",
    )
