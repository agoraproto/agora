"""Sprint 21 — Bau-Compliance Agent provider loop.

Polls /v1/jobs for jobs offered to this agent's DID, runs the task
through Claude Haiku with the German building/renovation compliance
system prompt, parses the JSON output, and submits the result on-chain
via the SDK's x402 helper (submitResult → backend mirror).
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [bau] %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger("bau_compliance_agent")


# ─────────────────────────────────────────────────────────────────────
# LLM call (larger max_tokens than swarm.lib.llm allows)
# ─────────────────────────────────────────────────────────────────────
...
async def call_claude(system_prompt: str, task_spec: dict[str, Any]) -> str:
    ...
    if not api_key:
        ...
        return json.dumps({
            ...
            "summary": {
                "scenario_excerpt": str(task_spec.get("scenario", ""))[:120],
                "applicable_rules": [],
                "obligations": [],
                "available_subsidies": [],
                "open_questions": ["Configure ANTHROPIC_API_KEY on the agent host."],
                "estimated_max_subsidy_pct": None,
                "top_next_steps": [],
            },
        })
    ...
    if r.status_code != 200:
        ...
        return json.dumps({
            ...
            "summary": {
                "scenario_excerpt": str(task_spec.get("scenario", ""))[:120],
                "applicable_rules": [], "obligations": [], "available_subsidies": [],
                "open_questions": [], "estimated_max_subsidy_pct": None,
                "top_next_steps": [],
            },
        })
    data = r.json()
...
class BauComplianceAgent:
    ...
    async def run(self) -> None:
        log.info("bau compliance agent online (did=%s, addr=%s, poll=%ds)",
    ...
    async def _handle_job(self, job: dict) -> None:
        ...
        log.info("handling job %s (focus=%s, scenario_len=%d)",
                 jid, task_spec.get("focus", "all"),
                 len(str(task_spec.get("scenario", ""))))
        ...
        try:
            ...
            steps = len(result_payload.get("summary", {}).get("top_next_steps", []))
            subs = len(result_payload.get("summary", {}).get("available_subsidies", []))
            log.info("job %s submitted: status=%s steps=%d subsidies=%d",
                     jid, res.get("status"), steps, subs)
        except Exception as e:
